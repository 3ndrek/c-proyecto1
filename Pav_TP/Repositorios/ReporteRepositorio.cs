...
namespace Pav_TP.Repositorios
{
    public class ReporteRepositorio
    {
        ...
        public DataTable ReporteFiltros(ReporteFiltros r)
        {
        ...
        {
             var sql = $"SELECT Count(r.nro_reservacion) as Cantidad, p.nombre as PaisSalida FROM reservaciones r JOIN viaje v ON r.cod_navio = v.cod_navio AND r.fecha_viaje = v.fecha_viaje JOIN puertoXitinerarios pXi ON v.cod_itinerario = pXi.cod_itinerarios JOIN puertosXpaises pXp ON pXi.cod_puerto = pXp.cod_puerto" +
                      $" JOIN paises p ON pXp.cod_pais = p.cod_pais" +
                      $" JOIN itinerarios i ON i.cod_itinerario = v.cod_itinerario" +
                      $" JOIN categoriasItinerarios ci ON ci.categoria = i.categoria" +
